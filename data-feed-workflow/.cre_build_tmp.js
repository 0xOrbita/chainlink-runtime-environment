...
var oracleSchema = exports_external.object({
  name: exports_external.string(),
  address: exports_external.string(),
  feedId: exports_external.string()
});
var configSchema = exports_external.object({
  ...
  priceConsumerAddress: exports_external.string().default(""),
  stalenessThresholdSeconds: exports_external.number().default(3600),
  deviationThresholdBps: exports_external.number().default(100),
  enableWrite: exports_external.boolean().default(false),
  oracles: exports_external.array(oracleSchema)
});
...
function hasDeviation(newAnswer, oldAnswer, thresholdBps) {
  if (oldAnswer === 0n)
    return true;
  const diff = newAnswer > oldAnswer ? newAnswer - oldAnswer : oldAnswer - newAnswer;
  const deviationBps = diff * 10000n / oldAnswer;
  return deviationBps >= BigInt(thresholdBps);
}
function readOraclePrice(runtime2, evmClient, oracle) {
  if (!oracle.address || oracle.address === `0x${"0".repeat(40)}`) {
    runtime2.log(`[SKIP] ${oracle.name}: address not configured`);
    return null;
  }
  try {
    ...
    const decimalsResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: oracle.address,
    ...
    runtime2.log(`[ERROR] ${oracle.name}: read failed - ${String(err)}`);
    return null;
  }
}
function updateOraclePrice(runtime2, evmClient, oracle, newPrice, timestamp) {
  try {
    ...
    if (resp.txStatus !== TxStatus.SUCCESS) {
      runtime2.log(`[ERROR] ${oracle.name}: setPrice tx failed - hash=${resp.txHash}`);
      return false;
    }
    runtime2.log(`[WRITE] ${oracle.name}: setPrice(${newPrice}, ${timestamp}) ok - tx=${resp.txHash}`);
    return true;
  } catch (err) {
    runtime2.log(`[ERROR] ${oracle.name}: setPrice failed - ${String(err)}`);
    return false;
  }
}
function readStoredPrice(runtime2, evmClient, feedId, priceConsumerAddress) {
  if (!priceConsumerAddress)
    return null;
  try {
    ...
    const tsResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: priceConsumerAddress,
        data: tsCallData
      }),
      blockNumber: LAST_FINALIZED_BLOCK_NUMBER
    }).result();
...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log(`[${ts}] === Orbita CRE PriceFeed cycle start ===`);
  ...
  for (const oracle of config.oracles) {
    runtime2.log(`[READ] ${oracle.name} @ ${oracle.address}`);
    const data = readOraclePrice(runtime2, evmClient, oracle);
    if (!data) {
      skipped++;
      continue;
    }
    ...
    runtime2.log(`[PRICE] ${oracle.name}: $${formatted} | ` + `decimals=${data.decimals} | updatedAt=${updatedAtDate} | stale=${stale}`);
    const last = lastKnownPrices.get(oracle.name);
    const deviated = last ? hasDeviation(data.answer, last.answer, config.deviationThresholdBps) : true;
    if (stale)
      runtime2.log(`[STALE] ${oracle.name}: >${config.stalenessThresholdSeconds}s old`);
    if (deviated && last) {
      const bps = (data.answer > last.answer ? data.answer - last.answer : last.answer - data.answer) * 10000n / last.answer;
      runtime2.log(`[DEVIATION] ${oracle.name}: ${Number(bps) / 100}% change`);
    }
    if (config.priceConsumerAddress && oracle.feedId) {
      const stored = readStoredPrice(runtime2, evmClient, oracle.feedId, config.priceConsumerAddress);
      if (stored) {
        runtime2.log(`[CONSUMER] ${oracle.name}: stored price=${stored.price} ts=${stored.timestamp}`);
      }
    }
    const needsUpdate = stale || deviated;
    if (needsUpdate && config.enableWrite) {
      runtime2.log(`[WRITE] ${oracle.name}: calling setPrice...`);
      const ok = updateOraclePrice(runtime2, evmClient, oracle, data.answer, nowSeconds);
      if (ok) {
        updated++;
      ...
      runtime2.log(`[SKIP-WRITE] ${oracle.name}: needs update but enableWrite=false`);
      lastKnownPrices.set(oracle.name, {
        answer: data.answer,
      ...
      runtime2.log(`[OK] ${oracle.name}: no update needed`);
      lastKnownPrices.set(oracle.name, {
        answer: data.answer,
        updatedAt: data.updatedAt
      });
  ...
  runtime2.log(`[DONE] updated=${updated} skipped=${skipped} ts=${ts}`);
  return "complete";
};
