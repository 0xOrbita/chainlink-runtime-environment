...
var poolSchema = exports_external.object({
  lendingPool: exports_external.string(),
  router: exports_external.string(),
  borrowToken: exports_external.string()
});
var configSchema = exports_external.object({
  schedule: exports_external.string().default("*/30 * * * * *"),
  chainSelectorName: exports_external.string().default("ethereum-testnet-sepolia-worldchain-1"),
  helperAddress: exports_external.string(),
  indexerUrl: exports_external.string(),
  enableLiquidation: exports_external.boolean().default(false),
  pools: exports_external.array(poolSchema)
});
function formatUsd(value2) {
  const whole = value2 / 10n ** 18n;
  const frac = value2 % 10n ** 18n * 100n / 10n ** 18n;
  return `$${whole}.${frac.toString().padStart(2, "0")}`;
}
async function fetchBorrowersFromIndexer(indexerUrl, lendingPool) {
  const query = `{
  ...
  try {
    ...
    return (json?.data?.borrowDebts?.items ?? []).filter((item) => item.lendingPoolAddress.toLowerCase() === lendingPoolLower && BigInt(item.amount ?? "0") > 0n).map((item) => item.user);
  } catch {
    return [];
  }
}
function checkHealth(runtime2, evmClient, borrower, lendingPool, helperAddress) {
  try {
    const callData = encodeFunctionData({
      abi: HELPER_ABI,
      functionName: "isLiquidatable",
      args: [borrower, lendingPool]
    });
    ...
    const [liquidatable, borrowValue, collateralValue, bonus] = decodeFunctionResult({
      abi: HELPER_ABI,
      functionName: "isLiquidatable",
    ...
    runtime2.log(`[ERROR] isLiquidatable(${borrower}): ${String(err)}`);
    return null;
  }
}
function approveToken(runtime2, evmClient, tokenAddress, spender) {
  try {
    const callData = encodeFunctionData({
      abi: ERC20_ABI,
    ...
    if (resp.txStatus !== TxStatus.SUCCESS) {
      runtime2.log(`[ERROR] approve failed - tx=${resp.txHash}`);
      return false;
    }
    runtime2.log(`[APPROVE] token=${tokenAddress} spender=${spender} tx=${resp.txHash}`);
    return true;
  } catch (err) {
    runtime2.log(`[ERROR] approve: ${String(err)}`);
    return false;
  }
}
function executeLiquidation(runtime2, evmClient, borrower, lendingPool) {
  try {
    const callData = encodeFunctionData({
    ...
    const resp = evmClient.writeReport(runtime2, {
      receiver: lendingPool,
      report: report2
    }).result();
    if (resp.txStatus !== TxStatus.SUCCESS) {
      runtime2.log(`[ERROR] liquidation(${borrower}) failed - tx=${resp.txHash}`);
      return false;
    }
    runtime2.log(`[LIQUIDATED] borrower=${borrower} pool=${lendingPool} tx=${resp.txHash}`);
    return true;
  } catch (err) {
    runtime2.log(`[ERROR] liquidation(${borrower}): ${String(err)}`);
    return false;
  }
}
var onCronTrigger = async (runtime2) => {
  ...
  runtime2.log(`[${ts}] === Liquidation cycle start ===`);
  ...
  if (!network248) {
    runtime2.log(`[ERROR] Network not found: ${config.chainSelectorName}`);
    return "error";
  }
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const approvedPools = new Set;
  let totalChecked = 0;
  let totalLiquidated = 0;
  for (const pool of config.pools) {
    runtime2.log(`[POOL] lendingPool=${pool.lendingPool}`);
    const borrowers = await fetchBorrowersFromIndexer(config.indexerUrl, pool.lendingPool);
    runtime2.log(`[POOL] ${borrowers.length} active borrowers`);
    for (const borrower of borrowers) {
      totalChecked++;
      const health = checkHealth(runtime2, evmClient, borrower, pool.lendingPool, config.helperAddress);
      if (!health)
        continue;
      const { liquidatable, borrowValue, collateralValue, bonus } = health;
      runtime2.log(`[HEALTH] borrower=${borrower}` + ` | liquidatable=${liquidatable}` + ` | borrowValue=${formatUsd(borrowValue)}` + ` | collateralValue=${formatUsd(collateralValue)}` + ` | bonus=${formatUsd(bonus)}`);
      if (!liquidatable) {
        runtime2.log(`[OK] ${borrower}: healthy`);
        continue;
      }
      runtime2.log(`[LIQUIDATABLE] ${borrower}: borrow=${formatUsd(borrowValue)} > collateral threshold`);
      if (!config.enableLiquidation) {
        runtime2.log(`[SKIP] enableLiquidation=false`);
        continue;
      }
      if (!approvedPools.has(pool.lendingPool)) {
        const ok = approveToken(runtime2, evmClient, pool.borrowToken, pool.lendingPool);
        if (!ok) {
          runtime2.log(`[ERROR] approve failed, skipping pool`);
          break;
        }
        approvedPools.add(pool.lendingPool);
      }
      const success = executeLiquidation(runtime2, evmClient, borrower, pool.lendingPool);
      if (success)
        totalLiquidated++;
    }
  }
  runtime2.log(`[DONE] checked=${totalChecked} liquidated=${totalLiquidated} ts=${ts}`);
  return "complete";
};
